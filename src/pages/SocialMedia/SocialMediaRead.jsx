...
export default function SocialMediaTable() {
  ...
  const handleSaveEdit = async (updatedUsernames) => {
    try {
      ...
      if (updatedUsernames.telegram) params.append("telegram_user_name", updatedUsernames.telegram);
      if (updatedUsernames.instagram) params.append("instagram_user_name", updatedUsernames.instagram);
      if (updatedUsernames.facebook) params.append("facebook_user_name", updatedUsernames.facebook);
      if (updatedUsernames.youtube) params.append("youtube_user_name", updatedUsernames.youtube);
      if (updatedUsernames.twitter) params.append("twitter_user_name", updatedUsernames.twitter);
  ...
  <meta name="description" content="Instagram, Telegram, YouTube, Facebook va X (Twitter) uchun obunachi yig‘ish uchun linklaringizni ko‘ring va boshqaring." />
</Helmet>
    <Card className="w-full max-w-[900px] mx-auto mt-4 ml-2">
      <CardHeader floated={false} shadow={false} className="rounded-none">
        <div className="flex items-center justify-between gap-8 flex-wrap">
          <Typography variant="h5" color="blue-gray">{t("socialMediaLinks")}</Typography>
          <div className="flex gap-4">
            <Button className="flex items-center gap-3" size="sm" onClick={() => setOpenEdit(true)}>
              <PencilIcon strokeWidth={2} className="h-4 w-4" /> Edit
            </Button>
            <Button className="flex items-center gap-3" size="sm" onClick={() => setOpenAdd(true)}>
      ...
      <CardBody className="px-0">
        <table className="w-full min-w-max table-auto text-left">
          <thead>
            <tr>
              <th className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4">Social Media</th>
              <th className="border-y border-blue-gray-100 bg-blue-gray-50/50 p-4">Username</th>
            </tr>
          </thead>
          <tbody>
            {Object.entries(usernames).map(([platform, username]) => (
              username && (
                <tr key={platform}>
                  <td className="p-4 border-b border-blue-gray-50">{platform.charAt(0).toUpperCase() + platform.slice(1)}</td>
                  <td className="p-4 border-b border-blue-gray-50">{username}</td>
                </tr>
              )
            ))}
          </tbody>
